...
// returns 0 on success, -1 on OOM. caller is responsible for error handling.
int tein_vfs_register(const char *key, const char *content, unsigned int length) {
    ...
    if (!entry->content) { free(entry->key); free(entry); return -1; }
